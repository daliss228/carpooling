...
class _RouteDetailPageState extends State<RouteDetailPage> {
    ...
    return Positioned(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            Text('${_user.name} ${_user.lastName}',
                style: TextStyle(
  ...
  Widget _hourInput() {
    return Container(
      ...
      child: TextField(
        ...
        style: TextStyle(
            fontFamily: "WorkSansLight", fontSize: 15.0, color: Colors.black),
  ...
  Future<void> getDataRoute() async {
    final resultRoute =
        (await _dbRef.child("routes").child(_idRoute).once()).value;
    ...
    final resultDriver =
        (await _dbRef.child("users").child(_route.driver).once()).value;
    setState(() {
      ...
      _seatController.text =
          'Asientos disponibles: ${_user.car.seat.toString()}';
      _carController.text =
          '${_user.car.brand}, ${_user.car.model}. ${_user.car.registry}';
      _isloading = false;
    });
    if (_route.coordinates != null) {
      _kGooglePlex = CameraPosition(
          target: LatLng(_route.coordinates.lat, _route.coordinates.lng),
          zoom: 16);
      _markers.add(Marker(
          markerId: MarkerId("miMarker"),
          position: LatLng(_route.coordinates.lat, _route.coordinates.lng)));
    }
  }
  ...
  String _stringSchedule() {
    ...
    if (stringDays.endsWith(" - ")) {
      print('asd');
      stringDays =
          stringDays.replaceRange(stringDays.length - 3, stringDays.length, "");
    }
    return stringDays;
  }
}
